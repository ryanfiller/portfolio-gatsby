...
const LinkHeader = (props) => {
  ...
  // todo if more than one child, get first that is string and use it as link, print the rest 

  const text = props.children
  const Header = props.h || 'h2'

  return (
    <Header id={slugify(text)} >
      <a href={`#${slugify(text)}`} aria-hidden="true" className="anchor">
